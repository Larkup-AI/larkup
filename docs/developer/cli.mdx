...
`index` recursively loads supported text, PDF, Word, JSON, and source code files, then streams chunking and embedding progress with a terminal progress bar. It skips `.git`, `.larkup`, `node_modules`, `dist`, and `build` directories. The active Project's document and index-run records are shared with the Web UI, so an indexing run started here appears in the Data and Index views immediately.
...
Use one simple command for a folder, or narrow it to a file type when needed:

```bash
larkup index ./folder                 # all supported files
larkup index ./folder --pdf
larkup index ./folder --json
larkup index ./folder --media         # image, audio, and video
larkup index ./folder --video
larkup index ./folder --extension .json,.pdf
```

`larkup indexing` is an alias for `larkup index`.

## Configure AI models

Projects, model settings, and indexing can all be managed from the terminal. Changes are written to the same active Project configuration used by the Web UI.

```bash
larkup init product-docs
larkup ai-model --embedding openai/text-embedding-3-small --api-key "$OPENAI_API_KEY"
larkup ai-model --chat openai/gpt-5 --api-key "$OPENAI_API_KEY"
larkup ai-model list --type embedding
larkup ai-model list --type chat
larkup ai-model list --type vision
```

`--apikey` is accepted as a shorthand for `--api-key`. `larkup set ai-model` is an alias for configuring models.
When no model flag is provided, `larkup ai-model --api-key <key>` updates the active embedding model's API key.
...
| `larkup ai-model [list]`    | Show, list, or configure embedding, chat, and vision models.                                                                      |
