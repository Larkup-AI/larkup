...
<Tabs>
  <Tab title="Docker" icon="docker">
    The Docker image runs the Larkup web app.

    ```bash
    docker run --platform linux/amd64 -d --name larkup -p 4567:4567 -v larkup-data:/app/.larkup aboneda/larkup:latest && sleep 5 && open http://localhost:4567
    ```
    ...
    The single-container command includes the web app and media processing. To scrape websites locally, start the optional crawler stack from the repository instead:
