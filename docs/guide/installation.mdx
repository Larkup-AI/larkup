...
<Tabs>
  <Tab title="Docker" icon="docker">
    ...
    ```bash
    docker run --platform linux/amd64 -d --name larkup -p 4567:4567 aboneda/larkup:latest
    ```

    Then open `http://localhost:4567` (on macOS, run `open http://localhost:4567`; on Linux, run `xdg-open http://localhost:4567`). A container cannot open a browser on the host by itself.
  </Tab>
