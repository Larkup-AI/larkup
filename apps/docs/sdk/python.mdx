...
Use `mode="sequential"` for strict request ordering or rate-limited embedding providers. Parallel mode controls client request scheduling only; the server retains its configured index type, vector store, embedding model, and chunking strategy.
