...
Local LanceDB is ideal for development and a VPS with a persistent disk. It is not durable on Vercel or other serverless platforms. Before deploying there, switch **Settings → Storage** to **LanceDB → S3-compatible (Vercel / R2 / S3)** and re-index your local source documents. This stores LanceDB table files in your own S3-compatible bucket, so no LanceDB Cloud account is required.
