...
export async function queryCommand(question: string, options: { project?: string; topK?: string }) {
  await inProjectScope(options.project, async () => {
    ...
    const topK = Math.min(Math.max(Number(options.topK) || config.topK, 1), 20);
    ...
    hits.forEach((hit, i) => {
      ...
      log.info(
        `${log.fmt.cyan(`#${i + 1}`)} ${log.fmt.bold(hit.title || 'Untitled')} ${log.fmt.dim(`(${pct}%)`)}`,
      );
