...
import { sanitizeGeneratedOutput } from '@larkup/marketplace/output';
...
Return only the viewing notes. Never include hidden reasoning, analysis, or <think> blocks.
...
Return only the final summary. Never include hidden reasoning, analysis, or <think> blocks.
...
Return only the consolidated notes. Never include hidden reasoning, analysis, or <think> blocks.
...
async function generateAndTrack(
  ...
  const sanitized = sanitizeGeneratedOutput(text);
  if (!sanitized) throw new Error('The media synthesis model returned no final answer.');
  return sanitized;
}
