...
export function SearchIntegrationsSection() {
  ...
  async function handleSave(section: 'webSearch' | 'webCrawler' | 'proxy') {
    setSaving(section);
    try {
  ...
  async function setWebSearchEnabled(enabled: boolean) {
    ...
    setSaving('webSearchToggle');
    try {
      const response = await fetch('/api/config', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...data.config, webSearchEnabled: enabled }),
      });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold tracking-tight">Search & Scraping</h2>
          <p className="text-sm text-muted-foreground">
            Manage your web search and scraping integrations.
          </p>
        </div>
      </div>
              ...
              <Tooltip>
                <TooltipTrigger
                  render={
                    <span>
                      <Switch
                        checked={form.webSearchEnabled === true}
                        disabled={saving === 'webSearchToggle'}
                        aria-label="Enable web search for the Agent"
                ...
                <TooltipContent>
                  {form.webSearchEnabled
                    ? 'Web search is enabled for the running Agent.'
                    : 'Enable web search for the running Agent.'}
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          </div>
          ...
          {form.webSearchProvider !== 'local' && (
            <div className="space-y-1.5 pt-2">
              ...
              {verifyStatus.status === 'success' && (
                <p className="text-[11px] text-green-500 font-medium mt-1">✓ API Key Verified</p>
              )}
              {verifyStatus.status === 'error' && (
                <p className="text-[11px] text-red-500 font-medium mt-1">{verifyStatus.message}</p>
              )}
            </div>
          )}
        </CardContent>
