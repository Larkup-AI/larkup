...
export function MarketplaceSection({ embedded = false }: { embedded?: boolean }) {
  const router = useRouter();
  const { data, isLoading, mutate } = useSWR('/api/marketplace', fetcher, {
    refreshInterval: (data) =>
      data?.tools.some((t) => t.status === 'installing' || t.status === 'uninstalling') ? 2000 : 0,
  });
  ...
  const handleUninstall = useCallback(
    async (toolId: string) => {
      ...
      const toastId = toast.loading('Removing tool…', {
  ...
  const toolToDeleteObj = useMemo(
    () => tools.find((t) => t.id === toolToDelete),
    [tools, toolToDelete],
  );
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="relative">
          ...
          <Input
            ...
            className={cn(
              ...
              'focus:border-foreground/20 bg-white/70',
            )}
          />
        </div>
      ...
      <GenericAlert
        ...
        description={
          <>
            Are you sure you want to remove <strong>{toolToDeleteObj?.name}</strong>?
            <br />
            <br />
            Any related installed tools credentials and configurations will be removed too.
          </>
        }
        cancelText="Cancel"
        actionText="Remove"
...
function ToolRow({
  ...
  return (
    <div
      className={cn(
        'group flex min-h-24 items-center gap-4 rounded-xl border border-border bg-white/70 px-4 py-4 dark:bg-card/70',
