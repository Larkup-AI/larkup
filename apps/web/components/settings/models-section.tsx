...
export function ModelsSection() {
  ...
  const { data: chatStatus } = useSWR(statusKey, (url: string) =>
    fetch(url).then((r) => r.json()),
  );

  const embeddingModels: EmbeddingModelDescriptor[] =
    chatStatus?.availableEmbeddingModels ?? [];
  const EMBEDDING_BY_PROVIDER = useMemo(() => {
    return embeddingModels.reduce<Record<string, EmbeddingModelDescriptor[]>>(
      (acc, m) => {
  ...
  async function handleSave(section: "embedding" | "chat") {
    ...
    try {
      ...
      if (section === "embedding") {
        if (form.embeddingProvider !== undefined) payload.embeddingProvider = form.embeddingProvider;
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
              ...
              >
                ...
                <SelectContent>
                  {PROVIDER_LIST.filter(
                    (key) =>
                      (EMBEDDING_BY_PROVIDER[key] &&
                        EMBEDDING_BY_PROVIDER[key].length > 0) ||
          ...
          <div className="space-y-1.5">
            ...
            <Select
              value={
                isOtherEmbedding ? "other" : form.embeddingModelId || ""
              }
            ...
            {isOtherEmbedding && (
              <Input
                ...
                onChange={(e) =>
                  setForm({ ...form, embeddingModelId: e.target.value })
                }
                className="mt-2"
              />
            )}
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription className="text-xs">
            Powers the AI assistant that answers questions from your knowledge
            base. Leave blank to use the same provider as embedding.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            ...
            {isOtherChat && (
              <Input
                ...
                onChange={(e) =>
                  setForm({ ...form, chatModelId: e.target.value })
                }
                className="mt-2"
              />
            )}
