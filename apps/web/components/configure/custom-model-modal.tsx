  ...
  const handleTest = async () => {
    ...
    try {
      if (type === 'embedding') {
          ...
          body: JSON.stringify(
            type === 'vision'
              ? {
                  ...
                  visionApiKey: apiKey,
                  visionModelId: `custom:${modelName}`,
                  customVisionModels: [{ baseUrl, apiKey, modelName }],
                }
              : {
                  chatProvider: 'custom',
                  chatApiKey: apiKey,
        ...
        setVerified(true);
        toast.success(`Success! ${type === 'vision' ? 'Vision' : 'Chat'} model connected.`);
      }
    } catch (err) {
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-row justify-between sm:justify-between items-center gap-2">
        ...
        <div className="flex gap-2">
          ...
          <Button className="px-5" type="button" onClick={handleSave} disabled={!verified}>
            Add Model
          </Button>
        </div>
      </div>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={'max-w-xl'}>
        <DialogHeader>
          <DialogTitle>
            Custom {type === 'embedding' ? 'Embedding' : type === 'vision' ? 'Vision' : 'Chat'}{' '}
            Model
          </DialogTitle>
          <DialogDescription>
            Connect an OpenAI-compatible{' '}
            {type === 'embedding' ? 'embedding' : type === 'vision' ? 'vision' : 'chat'} model.
          </DialogDescription>
        </DialogHeader>
        <CustomModelForm
          type={type}
