...
export async function GET() {
  ...
  // When running inside Docker, don't shell out to `docker` CLI.
  // Instead, check if a sibling Firecrawl service is reachable.
  if (runtimeEnv === "docker") {
    const [state, sibling] = await Promise.all([
      refreshLocalStatus(),
      checkDockerSibling(),
    ])
...
export async function POST(req: Request) {
  ...
  // Docker mode: connect/disconnect to the sibling service (can't docker compose from inside)
  if (runtimeEnv === "docker") {
    if (action === "start") {
      ...
      // "stop" in Docker just clears the wiring — we can't actually stop the sibling container
