...
test.describe("Documents API (/api/documents)", () => {
  ...
  test("POST /api/documents — empty content returns 400", async ({
    request,
  }) => {
    const res = await request.post(`${BASE}/api/documents`, {
      data: {
  ...
  test("PATCH /api/documents — update a document", async ({ request }) => {
    ...
    const createRes = await request.post(`${BASE}/api/documents`, {
      data: {
        title: "To Be Updated",
    ...
    const res = await request.patch(`${BASE}/api/documents`, {
      data: {
        id: document.id,
        title: "Updated Title",
    ...
    // Cleanup
    await request.delete(`${BASE}/api/documents?id=${document.id}`);
  });

  test("PATCH /api/documents — missing id returns 400", async ({
    request,
  }) => {
    const res = await request.patch(`${BASE}/api/documents`, {
      data: { title: "No ID" },
  ...
  test("DELETE /api/documents?id=x — delete one document", async ({
    request,
  }) => {
    ...
    const res = await request.delete(
      `${BASE}/api/documents?id=${document.id}`
    );
    expect(res.status()).toBe(200);
    const body = await res.json();
  ...
  test("DELETE /api/documents?ids=a,b — delete multiple", async ({
    request,
  }) => {
    // Create two docs
    const doc1 = await (
      await request.post(`${BASE}/api/documents`, {
        data: {
    ...
    const res = await request.delete(`${BASE}/api/documents?ids=${ids}`);
    expect(res.status()).toBe(200);
  ...
  test.afterAll(async ({ request }) => {
    if (createdDocId) {
      await request
        .delete(`${BASE}/api/documents?id=${createdDocId}`)
        .catch(() => {});
    }
  });
});
