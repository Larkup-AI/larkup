...
test.describe("Config API (/api/config)", () => {
  test("GET /api/config returns current configuration", async ({ request }) => {
    ...
    console.log(`  ✓ Config: vectorStore=${body.config.vectorStore}, model=${body.config.embeddingModelId}`);
  });
  ...
  test("PUT /api/config with valid LanceDB config", async ({ request }) => {
    ...
    if (res.status() === 200) {
      ...
      console.log(`  ℹ LanceDB config PUT returned 422 (connection test): ${JSON.stringify(body.fieldErrors ?? body.error)}`);
    }
  });
  ...
  test("PUT /api/config with invalid embedding model returns 400", async ({
    request,
  }) => {
    const getRes = await request.get(`${BASE}/api/config`);
    const { config: current } = await getRes.json();
  ...
  test("PUT /api/config with invalid vector store returns 400", async ({
    request,
  }) => {
    const getRes = await request.get(`${BASE}/api/config`);
    const { config: current } = await getRes.json();
  ...
  test("PUT /api/config with missing Pinecone fields returns 422", async ({
    request,
  }) => {
    const getRes = await request.get(`${BASE}/api/config`);
    const { config: current } = await getRes.json();
