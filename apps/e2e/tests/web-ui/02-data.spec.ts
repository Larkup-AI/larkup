...
test.describe("Data Page", () => {
  ...
  test("upload PDF file", async ({ page }) => {
    ...
    await expect(
      page.getByText("demo.pdf").or(page.getByText("demo"))
  ...
  test("upload TXT file", async ({ page }) => {
    ...
    await expect(
      page.getByText("demo.txt").or(page.getByText("demo"))
    ).toBeVisible({ timeout: 15_000 });
  });
  ...
  test("upload DOCX file", async ({ page }) => {
    ...
    await expect(
      page.getByText("demo.docx").or(page.getByText("demo"))
    ).toBeVisible({ timeout: 15_000 });
  });
  ...
  test.skip("upload CSV file", async ({ page }) => {
    ...
    await expect(
      page.getByText("demo.csv").or(page.getByText("demo"))
    ).toBeVisible({ timeout: 15_000 });
  });
  ...
  test("web search with Serper", async ({ page }) => {
    test.skip(!hasEnv(ENV_KEYS.SERPER_API_KEY) || !hasEnv(ENV_KEYS.FIRECRAWL_CLOUD_API_KEY), "Search requires Serper and Crawling requires Firecrawl");
    test.setTimeout(60_000);
