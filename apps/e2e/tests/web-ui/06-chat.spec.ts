...
test.describe("Chat Page", () => {
  ...
  test("send a message and receive a response", async ({ page }) => {
    ...
    const chatInput = page.locator('textarea[placeholder*="Ask about your knowledge base" i]').first();
  ...
  test("multi-turn conversation", async ({ page }) => {
    ...
    if (await chatInput.isVisible()) {
      ...
      const messages = page.locator('[class*="message"], [class*="chat-bubble"], [role="log"] > div');
