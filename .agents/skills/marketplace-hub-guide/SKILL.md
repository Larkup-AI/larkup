...
- **Core Endpoints:**
  ...
  - `GET /v1/tools/:id/install.sh`: A shell script for curl-based installations (`curl -sL hub.larkup.de/v1/tools/doc-editor/install.sh | sh`).
...
_(Repeat for `doc-editor` and `clip-embeddings`)_
...
Once the Hub is deployed (e.g., `https://hub.larkup.de`), point the main application to it by setting the environment variable in `apps/web`:

```env
LARKUP_HUB_URL="https://hub.larkup.de"
```
