...
import { addDocument, corpusStats, readDocuments } from "@/core/documents-store"
import { createRun, runIndexer } from "@/core/indexing/indexer"
import { readRun } from "@/core/index-store"
import { generateServer } from "@/core/generator/generate-server"
import { emitToDisk } from "@/core/generator/server-runtime"
import { embedQuery } from "@/core/indexing/embedder"
...
async function cmdInit(args: Args) {
  const name = args._[0] ?? (typeof args.flags.name === "string" ? args.flags.name : "my-rag")
...
async function cmdConfig(args: Args) {
  await inServerScope(args.flags, async () => {
    ...
    log(`  chunking   ${config.chunking.chunkSize}/${config.chunking.chunkOverlap} (${config.chunking.strategy})`)
    log(`  top-k      ${config.topK}`)
    log(`  corpus     ${stats.docCount} docs · ${stats.charCount.toLocaleString()} chars`)
    log(`  indexed    ${run?.status === "completed" ? `yes (${run.totalChunks} chunks)` : "no"}`)
  })
}
...
async function cmdAddDoc(args: Args) {
  await inServerScope(args.flags, async () => {
    ...
    let title = typeof args.flags.title === "string" ? args.flags.title : undefined
    ...
    log(c.green(`✓ Added "${doc.title}" (${doc.charCount.toLocaleString()} chars)`))
    const stats = await corpusStats()
...
async function cmdIndex(args: Args) {
  await inServerScope(args.flags, async () => {
    ...
    if (docs.length === 0) {
      fail('Corpus is empty. Add documents first: pnpm rag add-doc --file <path>')
    }
    if (!getEmbeddingModel(config.embeddingModelId)) {
      fail(`Unknown embedding model "${config.embeddingModelId}". Set it in the Configure stage.`)
    }
    if (!process.env.AI_GATEWAY_API_KEY) {
      log(c.yellow("! AI_GATEWAY_API_KEY is not set — embedding will likely fail."))
    }

    log(c.cyan(`Indexing ${docs.length} document(s) into ${config.vectorStore}…`))
...
async function cmdServe(args: Args) {
  await inServerScope(args.flags, async () => {
    ...
    log(c.green(`✓ Serving ${c.bold(config.projectName)} on http://localhost:${server.port}`))
    log(c.dim("  POST /query  ·  GET /health  ·  Ctrl+C to stop"))

    // Foreground: inherit stdio so logs stream and Ctrl+C stops the server.
    const child = spawn("node", ["server.mjs"], { cwd: dir, stdio: "inherit", env })
...
async function cmdQuery(args: Args) {
  await inServerScope(args.flags, async () => {
    ...
    hits.forEach((hit, i) => {
      ...
      log(`${c.cyan(`#${i + 1}`)} ${c.bold(hit.title || "Untitled")} ${c.dim(`(${pct}%)`)}`)
