...
test.describe('Index API (/api/index)', () => {
  test('GET /api/index — check indexing status and readiness', async ({ request }) => {
    ...
    if (body.ready) {
      console.log(`  ✓ Index ready: ${body.docCount} docs, running=${body.running}`);
    } else {
  ...
  test('POST /api/index — trigger indexing (if ready)', async ({ request }) => {
    ...
    if (!status.ready) {
      console.log('  ⏭ Skipping indexing — not ready: ' + status.blockers?.join(', '));
      test.skip(true, 'Indexing not ready');
      return;
    }

    if (status.docCount === 0) {
      console.log('  ⏭ Skipping indexing — corpus is empty (docCount=0)');
      test.skip(true, 'Corpus is empty');
      return;
    }
    ...
    // In fully parallel mode, another test might have just deleted the last document
    if (res.status() === 400) {
      const errorBody = await res.json();
      if (errorBody.error?.includes('corpus is empty')) {
        console.log('  ⏭ Skipping indexing — corpus was emptied by another parallel test');
        test.skip(true, 'Corpus was emptied by another parallel test');
        return;
      }
    }
    ...
    while (attempts < maxAttempts) {
      ...
      let pollRes;
      try {
        pollRes = await request.get('/api/index');
      } catch (err: any) {
      ...
      if (pollBody.run?.status === 'failed') {
        console.error(`  ✗ Indexing failed: ${pollBody.run.error}`);
        break;
      }
  ...
  test('DELETE /api/index — cancel running index', async ({ request }) => {
    const res = await request.delete('/api/index');
    expect(res.status()).toBe(200);
    const body = await res.json();
