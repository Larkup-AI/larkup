...
4. **Analyze sequences** → A vision LLM connects actions, OCR, and timestamped context
...
tool never silently falls back to OpenAI. With Deepgram, `auto` uses provider language detection;
set an explicit code such as `ar`, `ar-EG`, `en`, or `de` when needed.
...
| Audio language | `auto`  | Detect the language, or use an explicit code |
