...
beforeEach(() => {
  ...
  listMock.mockReset();
  createMock.mockResolvedValue({ process: { executeCommand: executeCommandMock }, delete: deleteMock });
});

describe('daytonaAdapter.verifyCredentials', () => {
  it('requires an API key', async () => {
    await expect(daytonaAdapter.verifyCredentials({})).rejects.toThrow('Daytona API key is required.');
  });
...
describe('daytonaAdapter.execute — merged stdout/stderr', () => {
  it('puts successful output in stdout only', async () => {
    ...
    const result = await daytonaAdapter.execute({ code: 'print(1)', language: 'python' }, { apiKey: 'k' });
  ...
  it('mirrors the combined output into stderr when the command fails', async () => {
    ...
    const result = await daytonaAdapter.execute({ code: 'raise ValueError()', language: 'python' }, { apiKey: 'k' });
