...
function safeEqual(a: string, b: string): boolean {
  ...
  // Avoid length-mismatch exceptions before timing-safe comparison.
...
export const webhookChannel: ChannelAdapter = {
  ...
  testHint: 'POST a test message to the webhook URL below and check the response.',
  connectionUi: {
    requiresPublicIngress: true,
    endpointHint:
      'Your service posts to Larkup’s public webhook URL. Start a public tunnel when this Agent runs locally.',
  },
    ...
    return {
      externalMessageId:
        typeof body.messageId === 'string' && body.messageId
          ? body.messageId
          : // The signature identifies retries with the same timestamp and body.
  ...
  async send(message: OutboundMessage, settings): Promise<DeliveryResult> {
    ...
    // The route returns the response when no callback is configured.
