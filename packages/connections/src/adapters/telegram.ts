...
export const telegramChannel: ChannelAdapter = {
  ...
  testHint: 'Send your bot a direct message on Telegram — it should reply within a few seconds.',
  connectionUi: {
    ...
    endpointHint:
      'Your bot sends updates to Larkup’s public webhook URL. Start a public tunnel when this Agent runs locally.',
    publicIngressDescription:
      'Telegram sends updates directly to this HTTPS address. Larkup registers it for you when you save.',
    contact: {
      directMessage: 'Open your Telegram bot and send it a message.',
      channelMessage:
        'For a group, add the bot. Disable privacy mode in BotFather if it should read every group message; otherwise reply to the bot or use a command.',
    },
  },
  ...
  configFields: [
    {
      ...
      help: 'Open @BotFather, send /newbot, then paste the token here. Treat it as a password.',
      helpUrl: 'https://web.telegram.org/a/#93372553',
    },
    {
      key: 'webhookSecret',
  ...
  parse(request: InboundRequest): NormalizedMessage | null {
    ...
    // Ignore edits and channel posts after acknowledging them.
    ...
    return {
      ...
      // Describe attachments until media ingestion is available.
      text:
        text ||
  ...
  async health(settings) {
    ...
    try {
      ...
      const payload = (await response.json()) as { result?: { id?: number; username?: string } };
      const username = payload.result?.username;
      const testUrl =
        username && /^[A-Za-z0-9_]+$/.test(username) ? `https://t.me/${username}` : undefined;
      return {
        status: 'ok',
        detail: username ? `Connected as @${username}.` : 'Connected.',
        identity: username ? `@${username}` : undefined,
        externalId: payload.result?.id !== undefined ? String(payload.result.id) : undefined,
        testUrl,
      };
    } catch (error) {
      return {
