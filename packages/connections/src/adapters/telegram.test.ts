...
describe('telegram health', () => {
  ...
  it('returns the bot identity and a portable direct-message URL', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(
        new Response(
          JSON.stringify({ ok: true, result: { id: 8828185138, username: 'larkup_bot' } }),
          {
            status: 200,
    ...
    expect(await telegramChannel.health(SETTINGS)).toEqual({
      status: 'ok',
      detail: 'Connected as @larkup_bot.',
      identity: '@larkup_bot',
      externalId: '8828185138',
      testUrl: 'https://t.me/larkup_bot',
    });
  });
});

describe('telegram webhook registration', () => {
  it('registers the public URL and its generated secret without dropping queued updates', async () => {
    ...
    await expect(
      telegramChannel.registerWebhook?.(
        'https://agent.ngrok.app/api/connections/telegram',
        SETTINGS,
      ),
    ).resolves.toMatchObject({ ok: true });

    const [url, init] = fetchMock.mock.calls[0];
    expect(url).toBe('https://api.telegram.org/bot123:AA/setWebhook');
    expect(JSON.parse(init.body)).toEqual({
      url: 'https://agent.ngrok.app/api/connections/telegram',
      secret_token: 'shh',
