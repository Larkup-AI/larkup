...
describe('slack parse', () => {
  ...
  it('normalizes a group app mention and removes the bot mention from its prompt', () => {
    const mention = messageEvent({ type: 'app_mention', text: '<@UBOT123> Where is my order?' });
    expect(slackChannel.parse(request(mention), SETTINGS)).toMatchObject({
      conversationId: 'C0123456',
      endUserId: 'U0123456',
      text: 'Where is my order?',
    });
...
describe('slack inbound interception', () => {
  it('declares the URL verification reply on the adapter, not in a route switch', () => {
    expect(
      slackChannel.interceptInbound?.(request({ type: 'url_verification', challenge: 'abc' })),
    ).toEqual({
      body: { challenge: 'abc' },
    });
    expect(slackChannel.interceptInbound?.(request(messageEvent()))).toBeNull();
  });
});
