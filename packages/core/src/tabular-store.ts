...
export function analyzeColumns(
  ...
  return columnNames.map((name) => {
    ...
    // Compute date range for date columns so the LLM knows the format and bounds
    if (type === "date") {
      ...
      if (timestamps.length > 0) {
        ...
        meta.dateRange = {
          min: dateStrings[timestamps.indexOf(sorted[0])] ?? new Date(sorted[0]).toISOString().split("T")[0],
          max: dateStrings[timestamps.indexOf(sorted[sorted.length - 1])] ?? new Date(sorted[sorted.length - 1]).toISOString().split("T")[0],
...
export async function getTabularDataset(
  id: string,
): Promise<TabularDataset | null> {
  const all = await readAll()
...
export async function queryTabular(
    ...
    for (const filter of query.filters) {
      ...
      rows = rows.filter((row) => {
        ...
        if (isDateCol && ["eq", "neq", "gt", "gte", "lt", "lte"].includes(filter.op)) {
          ...
          switch (filter.op) {
            case "eq":  return rowDate === filterDate
            case "neq": return rowDate !== filterDate
  ...
  // 3. Simple aggregations (no groupBy)
  ...
  if (
    ...
    for (const agg of query.aggregations) {
      const values = rows
        .map((r) => Number(r[agg.column]))
        .filter((n) => !isNaN(n))
      aggregationResults[`${agg.op}_${agg.column}`] = computeAgg(
        agg.op,
        values,
      )
    }
  ...
  const columns =
    rows.length > 0
      ? Object.keys(rows[0])
      : query.columns ?? dataset.columns.map((c) => c.name)
