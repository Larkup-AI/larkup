...
async function moveIfExists(from: string, to: string) {
  ...
  try {
    ...
    // Cross-device or busy — fall back to recursive copy + remove.
...
async function migrateOrInit(): Promise<Workspace> {
  ...
  // Repoint the migrated config's local db path at the new per-server dir.
...
export function createServer(
  name: string,
): Promise<{ workspace: Workspace; server: ServerMeta }> {
