...
export interface EnsureManagedYtDlpOptions {
  /** Re-download the official binary after a source compatibility failure. */
...
export async function ensureManagedYtDlp(
  rootDir = process.cwd(),
  options: EnsureManagedYtDlpOptions = {},
): Promise<string> {
...
export async function importMediaUrl(
  ...
  if (isYouTube(parsed)) {
    ...
    try {
      ...
      // A long-lived managed yt-dlp binary can lose compatibility as video
      // hosts change their player signatures. Refresh it once before retrying;
      // explicitly configured binaries remain under the operator's control.
      options.onProgress?.({
        message: 'Source rejected the download; updating the YouTube downloader and retrying…',
      });
      try {
        await ensureManagedYtDlp(process.cwd(), { forceRefresh: true });
        output = await runYtDlp(
          [
            ...commonArgs,
            '--force-ipv4',
