...
For a complex chat question, the agent routes the question before answering: focused facts use ranked timestamped evidence, a request for every source-authored question or written item uses an unranked chronological scan, and people/timeline requests can use a revision-scoped aggregate retained outside the chat context. The source mapper separates content-bearing prompts from interactional speech and declarative clues, stores directly established grids, sequences, and collections independently from their readable prompt text, and records separately resolved instances of a reusable prompt when the source proves their boundaries. That lets chat distinguish a complete source scan from a partially legible layout instead of treating banter or hints as questions. A full-source result is marked complete only when every source-inventory window finished; otherwise the response stays explicitly partial. Answers and respondents attached to an inventoried question are retained only when nearby source evidence contains them. If the selected evidence is incomplete, the agent may request one authorized, bounded cloud re-analysis (maximum 30 seconds per request) and then reads the resulting evidence again. The worker seeks directly to the approved range for model inference; source transfer and scale-to-zero GPU startup can still add latency for large videos.
